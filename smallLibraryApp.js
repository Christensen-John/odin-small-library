...
function createBookCard(book) {
  ...
  let newBookReadButton = ``;
  if (book.read) {
    newBookReadButton = `<button onClick="toggleReadStatus(event)" class="toggleReadStatus" id="${book.title}">Not read it!</button>`;
  } else {
    newBookReadButton = `<button onClick="toggleReadStatus(event)" class="toggleReadStatus" id="${book.title}">Read it!</button>`;
  }
		...
		<div class="bookCard" id="${book.title}">
          ...
          book.title
			...
			<div class="cardContent">
				...
				<div class="hasReadContainer">${book.read ? `Read!` : `Unread!`}
				${newBookReadButton}
				</div>
			</div>
		</div>
...
function toggleReadStatus(event) {
  ...
  //Get the location in the library for the book to toggle
  let bookLoc = myLibrary.findIndex((book) => {
    return book.title === title;
  });

  //Toggle the read status
  let currentStatus = myLibrary[bookLoc].read;
  myLibrary[bookLoc].read = !currentStatus;
